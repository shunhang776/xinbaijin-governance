...
function makeEvent(taskId, eventType, index) {
  return {
    ...
    event_id: 'event-claude-repair-pipeline-' + String(index).padStart(2, '0') + '-' + eventType.toLowerCase().replace(/_/g, '-'),
    task_id: taskId,
...
describe('Claude repair handoff L4 pipeline integration', () => {
  it('moves from REVIEW_DENIED to REPAIR_REQUESTED using adapter-produced event', () => {
    const taskId = 'task-claude-repair-pipeline-001';
    const runId = 'run-claude-repair-pipeline-001';
    ...
    const repairEvents = claudeRepairHandoffToL4EventObjects(handoff, {
    ...
    expect(repairEvents[0].event_type).toBe('REPAIR_REQUESTED');
    expect(validateEvent(repairEvents[0]), JSON.stringify(validateEvent.errors)).toBe(true);
    ...
    expect(output.final_state).toBe('REPAIR_REQUESTED');
    expect(output.task_state[stateField]).toBe('REPAIR_REQUESTED');
